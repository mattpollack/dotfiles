...
M.config = {
  enabled = true,
  paused = false, -- temporary pause state
  auto_balance = true,
...
--- Excludes special windows like quickfix and code trace (winfixheight)
local function get_top_level_vertical_windows()
  ...
  for _, win in ipairs(wins) do
    ...
    if config.relative == '' then
      ...
      -- Exclude windows with winfixheight (like quickfix, code trace, etc.)
      if pos[1] == 0 and not vim.api.nvim_win_get_option(win, "winfixheight") then
...
function M.balance()
  if not M.config.enabled or M.config.paused then
    return
  end
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command('PaneBalancePause', function()
    M.config.paused = not M.config.paused
    print('Pane balancer:', M.config.paused and 'paused' or 'unpaused')
  end, { desc = 'Toggle pause/unpause pane balancer' })
